...
**⚠️ UNCOMMITTED-TO-MAIN WORK:** this session's step-2 work is on branch **`algorithm-evaluation`** (3 commits ahead of `main`: `e8af687` eval harnesses+multi-centroid+LTR, `b03a0e1` weight re-tune, `a40c679` writeup+README). **NOT merged, NOT pushed.** Next session decide: merge to main + push (auto-deploys to Vercel) — README already synced.
