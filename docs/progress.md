...
2. **Evaluation + learning harness (THE BIG ONE — algorithm core).**
...
3. **Algorithm writeup** — map Dijkstra/MST/k-NN/heap/binary-heap/union-find/TF-IDF+cosine + the new learning-to-rank to the code, backed by the 2a/2b numbers (NDCG vs baselines, ablation table, the learning result). Not just a code description.
4. **RAG as the showcase AI feature — strictly scoped** (Phase 5; read `docs/ai-rag.md`). `game_embeddings` exists (1536-dim placeholder, HNSW cosine). "Ask about the recs the algorithm already chose" — subordinate in the UI, never the entry point. AI still never ranks.
