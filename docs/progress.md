...
1. **Adaptive WEIGHTS** (not just taste mode): content-heavy weights for focused libraries, graph-leaning for diverse ones. Must NOT override users who picked a manual "Recommendation style" preset → add an **"Adaptive" preset option** (DECIDED 2026-06-16: the new DEFAULT) that triggers per-user weight selection; manual presets still override. Touches `preferences-options.ts` (`WEIGHT_PRESETS`/`DEFAULT_PRESET`, special-case "adaptive" since it has no static weights), `actions.ts` `weightsFromPreset`, `run.ts` (compute weights from `analyzeTasteDiversity` when adaptive), `page.tsx` preset chips. ← **THE REMAINING step-A2 item.**
