...
User asked whether to also collect a reason (free-text or preset) when a user rates a rec good/bad. DECIDED: **park it** — a reason only earns its place if it changes ranking, else it's cosmetic data ("what does it do?" → "nothing"). **Free-text rejected** (needs an LLM to parse → pushes toward "AI does the work", off-thesis). The defensible version, if revived: **structured preset reasons → a NEGATIVE-preference term** — symmetric to the existing positive `preferenceScore` (penalize candidates carrying tags the user explicitly rejected, e.g. "not my genre: Shooter"). Keeps `lib/reco` pure (new input + penalty term). Honest caveat: with ~1 real user it demonstrates a mechanism more than it moves the needle. Documented future extension, not built.
...
**⭐ The product is feature-complete + polished** — the pure engine (`lib/reco/`), the impure bridge (`lib/reco-data/`), the Steam/AI/Supabase integrations, the `app/` dashboard, the DB migrations, and the eval `scripts/` are all shipped and verified across ~63 source files.

**THIS SESSION (2026-07-05):** Reviewed + verified a **partner's cleanup commits** (`d645598`/`6741977`/`164a85e`: dead shadcn scaffold + deps/exports removed, eslint clean, seed-picker debounced-search fix, sequel grouping in search) — all green, main flow intact. Shipped: **per-card explanations** (always-on DETERMINISTIC reason line from `score_breakdown`, made game-specific via `matchedStyles` = game tags ∩ the user's detected styles; AI "Why this match?" now optional) · **graded 1–10 review score** (migration **0005** `user_tried_games.score`; replaces 👍/👎; scales taste pull by score) · **UI polish** (score-meter hover preview, dropped the ✨, deduped landing CTA, plainer "How it works" card) · **RAG chat ❌ DECIDED AGAINST** (off-thesis; `game_embeddings` reserved for a future ranking SIGNAL, never a chat) · feedback "reasons" parked · game-length parked PERMANENTLY · **`docs/PlayNext-Algorithm-Writeup.docx`** generated from the refreshed writeup (⚠️ fill the title-page placeholders) · **perf** concurrent catalog pagination (`loadCatalog` cold 17.7s → ~4.5s, lossless) · **modularity** `app/dashboard/page.tsx` 648 → 300 lines split into 6 focused server components + `format.ts`. tsc + eslint + 32/32 reco tests + build green throughout.
...
4. **RAG chat — ❌ DECIDED AGAINST (2026-07-05, with user).** We will NOT build a RAG/semantic chat. Rationale: a chat box makes the LLM the face of the app and invites the "just an AI wrapper" perception that contradicts the thesis (ranking is a real, evaluated algorithm; AI only explains). It's also the easiest thing to break — ask it "what should I play?" (breaks "AI never ranks"), or expose a hallucination / a contradiction with the engine's order. And it has no rigorous evaluation, diluting the rigor story. The honest, stronger answer is *"we deliberately chose not to."* The `game_embeddings` table (pgvector/HNSW) stays **reserved/unused** — a possible FUTURE on-thesis use is a *semantic similarity SIGNAL feeding the engine* (an algorithm improvement with its own evaluation), never a chat. If any more AI is wanted, the low-risk option is extending the grounded explainer to *"compare my top recs"* (no chat, no open-catalog retrieval). See `docs/algorithm-writeup.md` §6.
