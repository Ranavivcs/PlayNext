...
### ▶ NEXT SESSION — Adaptive engine step A2 (DECIDED with user)
1. **Adaptive WEIGHTS** (not just taste mode): content-heavy weights for focused libraries, graph-leaning for diverse ones. Must NOT override users who picked a manual "Recommendation style" preset → add an **"Adaptive" preset option** (likely the new default) that triggers per-user weight selection; manual presets still override. Touches `preferences-options.ts` (`WEIGHT_PRESETS`/`DEFAULT_PRESET`, special-case "adaptive" since it has no static weights), `actions.ts` `weightsFromPreset`, `run.ts` (compute weights from `analyzeTasteDiversity` when adaptive), `page.tsx` preset chips.
2. **Name the detected styles** in the dashboard line — instead of "a few different kinds of games", say e.g. "You play shooters, sports games and roguelikes…" by labeling each taste cluster with its top tag(s). Needs: from `clusterByMst` groups, pick each cluster's most distinctive tag (highest idf or most common). Persist labels in `recommendations.params` for display.
...
4. **RAG as the showcase AI feature — strictly scoped** (← NEXT candidate) (Phase 5; read `docs/ai-rag.md`). `game_embeddings` exists (1536-dim placeholder, HNSW cosine). "Ask about the recs the algorithm already chose" — subordinate in the UI, never the entry point. AI still never ranks.
5. **Un-park game-length** with a real source: **HowLongToBeat** (fuzzy name-match, slow backfill) or **IGDB** (time-to-beat, Twitch OAuth). Feed `games.median_playtime` + un-hide the ChipGroup.
