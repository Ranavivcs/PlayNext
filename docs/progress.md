...
## Game-length soft preference (2026-06-05) ✅ BUILT (tsc + build clean, 11/11 reco tests pass)
DECIDED with user: **soft preference folded into the existing Preference term** (not a new weight, not a hard filter) — median playtime is noisy, so soft is safest.
