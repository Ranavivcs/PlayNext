...
- **`app/dashboard/rec-card.tsx`:** `MATCH_PHRASES` (one short phrase per score term) + pure `matchReason(breakdown)` → the top 1–2 contributing terms in plain words ("Closely matches your games · linked to your favorites"), with a colored dot from the lead term. Rendered above the (still optional) AI blurb; the "✨ Why this match?" button is unchanged as the on-demand AI dive.
...
### 2. Graded 1–10 ratings wired to the engine — NEXT (needs migration 0005)
DECIDED with user: replace binary 👍/👎 on Reviewed games with a **1–10 integer** score (no decimals — false precision the engine can't use; we already cut 4→2 ratings once for that reason). Wiring (keeps `lib/reco` pure — only changes engine INPUTS): high score → folds into the taste vector with synthetic playtime scaled by score (stronger pull); score ≤ ~5 → exclusion (like 👎 today). Needs migration `0005` (numeric `score` column on `user_tried_games`) applied via the Supabase SQL editor; `loadUserContext` reads score (falling back to the legacy `rating` for old rows); `my-games-panel.tsx` gets a 1–10 control. — built next.
